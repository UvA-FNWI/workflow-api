...
public class JobRepository(IMongoDatabase database, IOptions<WorkerOptions> workerOptions) : IJobRepository
{
    ...
    public async Task<Job?> GetById(string id, CancellationToken ct)
    {
        var filter = Builders<Job>.Filter.Eq("_id", ObjectId.Parse(id));
    ...
    {
        var filter = string.IsNullOrWhiteSpace(instanceId)
