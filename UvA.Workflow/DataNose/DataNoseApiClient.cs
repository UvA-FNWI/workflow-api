...
public class DataNoseApiClient(IHttpClientFactory httpFactory) : IDataNoseApiClient
{
    ...
    public async Task<IEnumerable<string>> GetRolesByUser(string userId,CancellationToken ct = default)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new HttpRequestException($"GetRolesForUser failed: {(int)response.StatusCode} {response.ReasonPhrase}. Body: {body}");
        }
    ...
    public async Task<IEnumerable<ExternalUser>> SearchPeople(string query, CancellationToken ct = default)
    {
        ...
        {
            var body = await response.Content.ReadAsStringAsync(ct);
            throw new HttpRequestException($"SearchPeople failed: {(int)response.StatusCode} {response.ReasonPhrase}. Body: {body}");
        }
        ...
        return peopleEntries.Where(p => (!string.IsNullOrEmpty(p.EmployeeUvAnetId) || !string.IsNullOrEmpty(p.StudentId)) &&
